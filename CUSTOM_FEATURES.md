...
Candidate generation and selection are separate. A typed policy table defines lane, mandatory status, and display priority instead of deriving semantics from string key prefixes. The card and scene Details tab both show at most seven initial chips. Hovering the shared chip area opens a scroll-safe popup containing every generated candidate with its evidence detail, including candidates outside the seven-chip selection. `No Orgasm` and leaning reserve their slots, optional automatic events fill next, up to two activity-quality chips follow, and remaining space admits up to five contextual chips. Final visual order is GOAT, Facial, No Orgasm, Really Hot event, active orgasm event, activity quality, leaning, interaction, rare role, negative rating, Favorite Vatos, Mexican lineup, filler, Few highlights, then generic amount tags; evidence score and label break ties within a kind. A dormant Everybody Nuts candidate type/policy is retained solely to make later restoration of the commented generation block straightforward.
