...
// sourceWidth is the source video width. Returns false if sourceWidth <= lowQualityMarkerWidth
// (both modes produce the same result for small source videos).
func (t *GenerateMarkersTask) markerQualityMismatch(sceneHash string, seconds int, sourceWidth int) bool {
	...
	if t.ImagePreview {
		...
		if exists, _ := fsutil.FileExists(webpPath); exists {
			if t.webpWidthMismatch(webpPath, sourceWidth) {
				return true
			}
		}
	}
	...
	if t.VideoPreview && !t.ImagePreview {
		...
		if exists, _ := fsutil.FileExists(videoPath); exists {
			if t.videoWidthMismatch(videoPath, sourceWidth) {
				return true
			}
		}
	}
...
func (t *GenerateMarkersTask) deleteQualityMismatchedFiles(sceneHash string, seconds int, sourceWidth int) {
	if t.ImagePreview {
		webpPath := instance.Paths.SceneMarkers.GetWebpPreviewPath(sceneHash, seconds)
		if exists, _ := fsutil.FileExists(webpPath); exists {
			if t.webpWidthMismatch(webpPath, sourceWidth) {
...
func (t *GenerateMarkersTask) webpWidthMismatch(path string, sourceWidth int) bool {
	f, err := os.Open(path)
	if err != nil {
		return false
	}
	...
	if t.HighQualityMarkers {
		return !widthMatches(actualWidth, sourceWidth)
	}
...
func (t *GenerateMarkersTask) videoWidthMismatch(path string, sourceWidth int) bool {
