...
import {
  MarkerTopCriterion,
  IMarkerTopFilter,
} from "src/models/list-filter/criteria/marker-top";
...
interface IMarkerTopFilterProps {
  criterion: MarkerTopCriterion;
  setCriterion: (c: MarkerTopCriterion) => void;
}

interface IFilterEditorProps {
  filter: IMarkerTopFilter;
  index: number;
  onUpdate: (updates: Partial<IMarkerTopFilter>) => void;
  onDelete: () => void;
  canDelete: boolean;
}
    ...
    <Card className="mb-3">
      <Card.Header className="d-flex justify-content-between align-items-center py-2">
        <Badge variant="secondary">
          <FormattedMessage id="markers_filter.top" defaultMessage="Top" /> #
          {index + 1}
        </Badge>
        {canDelete && (
          <Button
...
export const MarkerTopFilter: React.FC<IMarkerTopFilterProps> = ({
  criterion,
  setCriterion,
}) => {
  const intl = useIntl();
  ...
  const onAddFilter = () => {
    const c = criterion.clone() as MarkerTopCriterion;
    c.addFilter();
    setCriterion(c);
  };
  ...
  const onUpdateFilter = (
    index: number,
    updates: Partial<IMarkerTopFilter>
  ) => {
    const c = criterion.clone() as MarkerTopCriterion;
  ...
  if (!hasGroups) {
    return (
      <div className="marker-top-filter text-muted">
        <p>
          <FormattedMessage
            id="add_marker_tags_first"
            defaultMessage="Add a Marker Tags filter first to enable Top filtering."
          />
        </p>
      </div>
    );
  ...
  return (
    <div className="marker-top-filter">
      <div className="mb-3 text-muted">
        <FormattedMessage
          id="marker_top_help"
          defaultMessage="Filter markers by top performer attributes. Each filter targets a specific marker tag group."
        />
      </div>
      ...
      >
        ...
        <FormattedMessage id="add_top_filter" defaultMessage="Add Top" />
      </Button>
    </div>
  );
};
