...
export class SceneCustomFiltersCriterion extends Criterion {
  ...
  public static readonly options = ["multiple_orgasms", "versatile_scenes"];
...
export const PerformerCustomFiltersCriterionOption = new CriterionOption({
  messageID: "custom_filters",
  type: "custom_filters" as CriterionType,
  makeCriterion: (o, config) => new PerformerCustomFiltersCriterion(config),
});

export class PerformerCustomFiltersCriterion extends Criterion {
  public value: string = "";
  private roleTagIds: IUIConfig['roleTagIds'];
  ...
  constructor(config?: ConfigDataFragment) {
    super(PerformerCustomFiltersCriterionOption);
    this.roleTagIds = getRoleTagIds(config);
  }
  ...
  public isValid(): boolean {
    return PerformerCustomFiltersCriterion.options.includes(this.value);
  }
