...
type SceneCounter interface {
	...
	OCountByStudioID(ctx context.Context, studioID int, depth *int, performerID *string) (int, error) // CUSTOM: added depth and performerID params
}
...
type ODateReader interface {
	...
	GetOVideoTimestamps(ctx context.Context, id int) ([]*float64, error) // CUSTOM
}
...
type OHistoryWriter interface {
	...
	AddOAtVideoTimestamp(ctx context.Context, id int, videoTimestamp float64) ([]time.Time, error) // CUSTOM
}
